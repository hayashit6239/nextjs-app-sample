...
export const HomeProductCardListPresentationMemo: FC<Props> = (props) => {
    ...
    return (
        <Flex direction="column" className="h-full gap-y-8">
            <ProductCategoryArea
                category="トップス"
                productCards={products.filter((x) => x.category === "clothes")}
            />
            <ProductCategoryArea
                category="本"
                productCards={products.filter((x) => x.category === "book")}
            />
            <ProductCategoryArea
                category="シューズ"
                productCards={products.filter((x) => x.category === "shoes")}
            />
        </Flex>
    );
};
...
export const HomeProductCardListPresentation = React.memo(
    HomeProductCardListPresentationMemo,
);
